...
Version: 0.0
Release: 0%{?dist}
Summary: gilmanos first-party code
License: GNU or MIT
...
Summary: Gilmanos sysctl helper
Requires: %{_cross_os}apiserver = %{version}-%{release}
...
Summary: Applies changed settings to a Gilmanos system
Requires: %{_cross_os}apiserver = %{version}-%{release}
%description -n %{_cross_os}thar-be-settings
%{summary}.

%package -n %{_cross_os}thar-be-updates
Summary: Dispatches Gilmanos update commands
Requires: %{_cross_os}apiserver = %{version}-%{release}
...
install -d %{buildroot}%{_cross_datadir}/gilmanos
